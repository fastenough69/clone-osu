...
int main(void){
    ...
    while(!WindowShouldClose()){
        ...
        if(start_flag && TimeToStart <= 0){
            ...
            bool click = II_logic(*temp, *sldTime); 
            if(CheckCollisionPointCircle(mouse, temp->center, temp->frame.width / 2.0f - 5) && click){
                PlaySound(actionClick);
                combo++;
                total += (int)((500.0f / (sldTime->value - temp->lifetime.time)) * (float)(combo * 0.1)) / ((sldSize->value / 1000.0f) + (sldTime->value / 10.0f));
            ...
            if((CheckCollisionPointRec(mouse, (Rectangle){repeat->crd.x, repeat->crd.y, repeat->frame.width, repeat->frame.height}) && click) || IsKeyPressed(KEY_A)){
                TimeToStart = 3.0;
                combo = 0;
                total = 0;
            }
        ...
        sldTime->value = sldTime->rec.width / 100;
        sldSize->value = sldSize->rec.width;
        BeginDrawing();
